...
- [ ] מבנה תיקיות + `index.html` בסיסי
- [ ] Split view layout עם RTL מלא
- [ ] חיפוש פשוט בספריא (`/api/name/`)
- [ ] שליפה והצגה של פסוק (`/api/v3/texts/`)
- [ ] ניקוי טעמים, שמירת ניקוד
- [ ] כפתור "הוסף לדף" → מוסיף מקור לפאנל ימני
- [ ] תיבה עצמאית (custom source)
- [ ] עריכת טקסט וכותרת אחרי הוספה
- [ ] מחיקה וסידור מקורות (drag-and-drop או חיצים)
- [ ] ייצוא PDF דרך `window.print()` עם CSS print
- [ ] auto-save של טיוטה פעילה ל-localStorage

### Phase 2 - מקורות מקושרים

- [ ] קריאה ל-`/api/related/` לכל מקור שנפתח
- [ ] קיבוץ הקישורים לפי קטגוריה (פרשנים / מדרשים / וכו')
- [ ] preview של מקור מקושר עם הרחבה ב-click
- [ ] כפתור "הוסף" לכל מקור מקושר
- [ ] Quick-add: "הוסף את כל פרשני התורה" ברמת קבוצה

### Phase 3 - דפדוף מלא בעץ

- [ ] טעינת `/api/index` ב-app init
- [ ] עץ ניווט: קטגוריה → ספר → פרק → פסוק
- [ ] היסטוריית ניווט (אחורה/קדימה)
...
- [ ] טעינת דף שמור
- [ ] מחיקה, שכפול
- [ ] "התחל דף חדש" (עם confirm)

### Phase 5 - הגדרות עיצוב

- [ ] בחירת פונט (4 אופציות)
- [ ] בחירת גודל גופן
- [ ] צ'קבוקס "כל מקור בעמוד חדש"
- [ ] מספור מקורות
- [ ] שולי דף
- [ ] מצב הדפסה שחור-לבן
