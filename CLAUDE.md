...
| אל | א-ל | רק `אֵ` עם **צירי** (שם ה'). `אֶ` עם **סגול** (מילת יחס "אל/ל") **לא** מותאם. **"אל" ללא ניקוד כלל לא מותאם** — בטקסט לא-מנוקד זו כמעט תמיד מילת יחס ("פנים אל פנים"), והכלל הישן יצר false positives. קידומת **בודדת** `ב/ה/ו/כ/ל/מ/ש` נתפסת (`לָאֵל`, `וְאֵל`), אך **לא** ערימת קידומות: מילים/שמות הפותחים באותיות-קידומת ומסתיימים ב-`אֵל` (שְׁמוּאֵל, שׁוֹאֵל, לְמוּאֵל) נחתכים שגוי אם מתירים `{0,3}`. lookbehind מונע התאמה בתוך ישראל / אלעזר / אלא. |
| צבאות | צבא-ות | תמיד. |
